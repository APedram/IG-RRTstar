...
N = 30000; % Maximum number of nodes
...
for ii=2:N
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%% Inserting a new node to the tree %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        while nbor_issue ~= 0
            ...
            [x, P, ra, rb, ang, ellipse_rect, nearest] = sample_x_P_randomly(bound, node,In_list_ID, radius_min, r_attempt, chi, obstacle_edge, obs_polyshape);
            ...
            [parent, value, nbor_issue, P_prime_parent2node_i] =...
                find_parent(x, P,node,R,alpha, r_attempt, obstacle_edge, chi, bound, num_props, prop, In_list_ID, nearest);
        end
        
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Lossless modification by utilizing output from find_parent
% %      not_lossless = check_lossless( node(parent).x, node(parent).P, x, P, R );        
% %         if not_lossless == true
% %            % replace P with loss-less P_prime_parent2node_i
% %            P = P_prime_parent2node_i;
% %            [ra, rb, ang, ellipse_rect] = error_ellipse(x,P,chi); 
% %         end
        ...
        [nbors_rw_ID, ~, ~, ~] = find_neighbors( x, P, node, R, alpha, r_attempt, In_list_ID, Is_only_neibor_ID);
        ...
        for jj = 1:num_rw_nbor
            ...
            % check collision 
            issue_flag2(jj) = psuedo_obs_check_line2_oct(node(ii), node_jj, obstacle_edge, R, chi, bound, num_props, prop);
        end
        ...
        if ~isempty(nbors_rw_no_issue_ID)
            ...
            [dist_node_i2xk, dInf_i2xk, P_prime_i2xk_list] = dist_ig_mat2(node(ii).x.', node(ii).P, x_nbors_mat, P_nbors, alpha, R);
...
% %                     % parent
% %                     not_lossless = check_lossless( node(ii).x, node(ii).P,...
% %                         node(rewired_better_ID(k)).x, node(rewired_better_ID(k)).P, R );  
% %                     
% %                     % if the transition is not loss-less
% %                     if not_lossless == true
% %                         % computes the loss-less covariance P_rew_upd
% %                         P_rew_upd = P_prime_i2xk_list(:,:,rewired_ID_in_nbors_rw(k));
% %                         
% %                         % log in the modified values
% %                             [ra_rew_upd, rb_rew_upd, ang_rew_upd, ellipse_rect_rew_upd] =...
% %                                 error_ellipse(node(rewired_better_ID(k)).x, P_rew_upd, chi);
% %                             node(rewired_better_ID(k)).P = P_rew_upd;
% %                             node(rewired_better_ID(k)).ra = ra_rew_upd;
% %                             node(rewired_better_ID(k)).rb = rb_rew_upd;
% %                             node(rewired_better_ID(k)).ang = ang_rew_upd;
% %                             node(rewired_better_ID(k)).ellipse_rect = ellipse_rect_rew_upd;
% %                     end
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% updating the descendants of rewired nodes%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                    
                    ...
                    for cnt=1:num_descendants_k
                        ...
                        [dmn, dInf_mn, P_prime_desc] = dist_ig_mat(xm.',Pm,xn.',Pn, alpha, R);
                        node(n).value=node(m).value+dmn;
                        
% %                         % loss-less modification is perfomred if needed 
% %                         not_lossless = check_lossless( node(m).x, node(m).P, node(n).x, node(n).P, R );        
% %                         if not_lossless == true
% %                                 P_desc = P_prime_desc;
% %                                 [ra_desc, rb_desc, ang_desc, ellipse_rect_desc] =...
% %                                     error_ellipse(node(n).x, P_desc, chi);
% %                                 node(n).P = P_desc;
% %                                 node(n).ra = ra_desc;
% %                                 node(n).rb = rb_desc;
% %                                 node(n).ang = ang_desc;
% %                                 node(n).ellipse_rect = ellipse_rect_desc;
% %                         end
                    end                    
                end
            end            
    ...
    if rem(ii,bnb_val) == 0
        if ~isempty(path) 
            [node, remove_ID{ii/bnb_val}] = branch_and_bound_2D(node,path,In_list_ID, alpha,R, ii, ini_st, dim); 
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
savename = ['data/Multi_N',num2str(N),'_alpha_',num2str(alpha),'_safety_', num2str(0.9)];
savename(savename=='.') = [];
save(savename)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
