function [parent, value, nbor_issue, P_prime_parent2node_i] =...
    find_parent( x, P, node, R, alpha, radius, obstacle_edge, chi, bound, num_props, prop, In_list_ID, nearest)
...
issue_flag = psuedo_obs_check_line_oct(x, P, node(nearest), obstacle_edge, R, chi, bound, num_props, prop);
...
[dig, ~, P_prime_list] = dist_ig_mat( x_nearest_mat, P_nearest_mat, x.', P, alpha, R);
parent = nearest;
...
[nbors_ID, dist_xk2node_i,~ , P_prime_nei_list] =...
    find_neighbors( x, P, node, R, alpha, radius, In_list_ID, Is_only_neibor_ID); 
...
% Finally, run "obstacle_check" for these bounding boxes. 
    ...
    for jj = 1:num_nbor
        ...
        [issue_flag(jj)] = psuedo_obs_check_line_oct(x, P, node_jj, obstacle_edge, R, chi, bound, num_props, prop);
